  ...
  try {
    ...
    while (stack.length) {
      ...
      if (stat.isDirectory()) {
        ...
        let regexpNames =
          /(export|import)?(\s+?\{\s+?)?.+(\s+?\}\s+?)?(from)?(\s+)?(\'|\")(..\/|.\/)(.+)?(\'|\")/gm
...
async function runSpawnCmd(value: string) {
  return new Promise((resolve, reject) => {
    ...
    tsc.on('error', (err: any) => {
      // console.error(err, value, 'Failed to start subprocess.')
      reject(err)
    })
  })
}
...
async function getConfigTsConfig() {
  ...
  if (!compilerOptions.module) {
    ...
    //   assert: { type: 'json' },
    // })
    const fileBaseJson = require(filePath)
    // const configJson = fileJson.default
    // const configJson = fileJson
    if (fileBaseJson.compilerOptions.module === 'commonjs') {
      await clearFileCmd(configJson.luaoOutDir)
      return runSpawnCmd('tsc')
    }
  }

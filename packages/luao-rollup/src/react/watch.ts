import {
  // rollup, ModuleFormat, OutputOptions,
  watch,
} from 'rollup'
...
const watchBuildConfig = async (
  entry: string,
  bundleOpts: IBundleOptions & OptionProp & watchOption
) => {
  ...
  const rollupConfigs = await getRollupConfig({
    ...
    env: 'development',
    outDir:
      bundleOpts.umd !== false && bundleOpts?.umd?.dir
        ? bundleOpts?.umd?.dir
        : 'dist',
  })
...
const watchRollup = async (opts: IBundleOptions[], options: OptionProp) => {
  ...
  const option = opts.map(async (bundleOpt: IBundleOptions & watchOption) => {
    ...
    if (Array.isArray(bundleOpt.entry)) {
      ...
      const config = entries.map(
        async (entry) =>
          await watchBuildConfig(entry, { ...bundleOpt, cwd, type })
      )
      watchOption = config
      signale.info(config, 'multi --- entry')
      signale.warn('multi-entry function watching not open now')
    } else {
      const config = await watchBuildConfig(bundleOpt.entry!, {
        ...bundleOpt,
        cwd,
        type,
      })
  ...
  Promise.all(option).then((options) => {
    ...
    watcher.on('close', () => {
      global.error('rollup:watcher: closed')
    })
    process.on('exit', () => {
      // 停止监听
      watcher.close()

...
async function getBundleOpts({
  entry,
}: {
  entry?: string
}): Promise<IBundleOptions[]> {
  entry =
    entry ??
    getExistFile([
      'src/index.tsx',
              ...
              new Promise((resolve) => {
                ...
                resolve(
                  rollup({
                    ...
                    codeBabelType: bundleOpt?.codeBabelType || 'react',
                    outDir:
                      bundleOpt.umd !== false && bundleOpt?.umd?.dir
                        ? bundleOpt?.umd?.dir
                        : 'dist',
                  }).then(() => {
